...
import (
	...
	slackinbound "github.com/temporal-community/temporal-agent-harness/nexus/ui_connector/slack/inbound"
...
func main() {
	...
	if botUserID == "" {
		...
		bot, err := slack.NewSlackBot(botToken)
		if err != nil {
	...
	server := slackinbound.NewServer(tc, taskQueue, signingSecret, botUserID, agentWFPrefix)
