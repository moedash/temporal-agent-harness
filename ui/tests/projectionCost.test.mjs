...
import { summarizeCost } from "$lib/cost/pricing.ts";
import { buildAgentTreeGraph } from "$lib/state/flowProjection.ts";
import { buildReplayLog } from "$lib/state/replayLog.ts";
import { buildReplayTimeline } from "$lib/state/replayTimeline.ts";
import { buildStepTimeline } from "$lib/state/stepTimeline.ts";
import { buildTranscript } from "$lib/state/transcript.ts";
...
describe("one commit stays O(frames)", () => {
  it("4x the events must not cost more than 8x the work", () => {
    ...
    /* The absolute constant is REPORTED, not asserted. This used to be a hard
       ...
       goes green is a check nobody reads the output of. So: report, do not gate.
       ...
       Reporting is the stopgap, not the answer. The absolute is load-sensitive
       because it is absolute; the fix is to make it comparative — measure a fixed
       synthetic workload in the same process as a yardstick and assert on
       commit/yardstick, which is a ratio and so immune the way the assertion above
       is. That was not built here because it cannot be calibrated on a machine this
       noisy: validating a load-normalisation scheme needs a quiet box, and the
       point of it is that we do not have one.

       The 120 records what a loaded machine cost, not a budget anyone endorsed —
       ...
       port landing now makes those viable. Moving the cliff means making
       buildReplayLog incremental — appending rows for arriving frames instead of
       rebuilding every row on every commit.
    ...
    /* Kept as a print, not converted to an assertion: the comment above is the
       reason this file no longer gates on the absolute, and a number nobody can
       read is the same as a number nobody recorded. */
    console.log(
