...
describe("thoughtDeltaText", () => {
  it("finds the thought in every shape a producer dumps", () => {
    assert.equal(
      Object.keys(SHAPES).length,
      4,
      "reading one shape is the defect, so every producer stays pinned: if a shape was added or " +
        "removed here, say so"
    );
  ...
  it("reads a payload with no text as silence", () => {
    ...
    assert.equal(
      thoughtDeltaText({ signature_delta: "sig" }),
      "",
      "a signature delta carries no text"
    );
...
describe("a thought card", () => {
  it("holds the whole thought on every surface that reads the frames", () => {
    for (const [provider, shape] of Object.entries(SHAPES)) {
      ...
      assert.equal(
        thoughts.length,
        3,
        `${provider}: the transcript should keep the thought fragments`
      );
    }
  });

  it("with no text says why, rather than rendering blank", () => {
    ...
    const silent = reasoningCard([
      ...
      frame("thought_summary", {
        delta: { signature_delta: "sig", part_delta_kind: "thinking_delta" }
      }),
  ...
  /* A run that asked for no summary at all — Reasoning(summary=None), as
     ...
     nothing. */
  it("is not drawn at all when the run asked for no summary", () => {
    assert.equal(
  ...
  it("says how long it thought", () => {
    ...
    assert.equal(
      timed.subtitle,
      "Thought for 3s",
      `the finished card should say how long: ${timed.subtitle}`
    );
...
describe("one extractor, not four", () => {
  it("is imported by every projection that reads a thought", () => {
    for (const path of ["./flowProjection.ts", "./replayLog.ts", "./transcript.ts"]) {
