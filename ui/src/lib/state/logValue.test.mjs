...
import AgentStateNode from "$lib/components/flow/AgentStateNode.svelte";
import { buildAgentGraph } from "./flowProjection.ts";
import { formatLogValue, UNKNOWN_TOOL_INPUT } from "./logValue.ts";
import { buildReplayLog } from "./replayLog.ts";

/* @xyflow/svelte is stubbed, so the connection handles AgentStateNode draws are not exercised
   here. Its Handle reads the store a live SvelteFlow canvas puts in context and throws without
   one, and standing a canvas up to render two decorative dots is a worse trade than naming the
   gap. The component under test is the real one — only this dependency is faked, and it
   contributes nothing to the text asserted below. A check that needs the handles themselves
   would have to render inside a real SvelteFlow. */
...
describe("unknown tool input", () => {
  ...
  it("reads as unknown rather than as a value", () => {
    assert.match(
      UNKNOWN_TOOL_INPUT,
      /^— /,
      "the unknown marker is a leading em dash, as formatCost uses"
    );
    assert.ok(
  ...
  it("is what the formatter both panels import says about null, and only null", () => {
  ...
  it("survives the real projections as null, not as undefined", () => {
    ...
    assert.equal(
      rowFor("call_UNKNOWN").input,
      null,
      "null must reach the row as null, not undefined"
    );
    assert.deepEqual(
      rowFor("call_BARE").input,
      {},
      "an argument-less call must reach the row as {}"
    );
  ...
  /* SSR the real component rather than asserting on the projection alone: `JSON.stringify(null)`
     ...
     model sent. */
  it("is what the flow node actually renders", () => {
  ...
  it("comes from one formatter, not two", () => {
    for (const path of [
      "../components/agent/TranscriptPanel.svelte",
      "../components/agent/AgentChatPanel.svelte"
    ]) {
