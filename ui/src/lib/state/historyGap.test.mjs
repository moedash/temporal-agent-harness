...
import { HISTORY_GAP_NOTE, findHistoryGaps } from "./historyGap.ts";
import { buildReplayLog } from "./replayLog.ts";
import { buildStepTimeline } from "./stepTimeline.ts";
...
describe("the defect: a stale cache spliced onto a later window", () => {
  const spliced = [
  ...
  it("finds the seam, and only the seam", () => {
  ...
  it("names it once in the log, with a note that invents no size", () => {
  ...
  it("never pairs a span across the seam", () => {
...
describe("ordinary continuous frames produce nothing", () => {
  it("finds no seam in a dense run of offsets", () => {
    const continuous = [
...
describe("the documented degradation is not a gap", () => {
  const coldLoad = [
    delta(4906, "nine hundred thousand, ", 4),
    modelEnd(4907, 4),
    frame("turn_end", 4908, {}, ROOT, 4)
  ...
  it("does not flag a window that merely starts late", () => {
    assert.deepEqual(
  ...
  it("still draws no span from the orphaned end frame it holds", () => {
    assert.equal(
...
describe("frames outside the root's offset space must not manufacture one", () => {
  it("ignores a subagent's own offset sequence", () => {
  ...
  it("does not read the SYNTHESIZED sentinel as an offset", () => {
  ...
  it("skips a frame with no offset rather than reading it as zero", () => {
  ...
  it("invents no seam from out-of-order arrival", () => {
...
describe("and nothing fires on the fixture every other check reads", () => {
  it("reads realisticQaScenario as continuous", () => {
    /* The frame count the summary line used to print, asserted instead: an empty fixture would
       satisfy the three claims below by never reaching a comparison at all. */
    assert.ok(
      realisticQaScenario.frames.length > 0,
      "the demo fixture must carry frames, or these three claims are vacuous"
    );
    assert.deepEqual(
