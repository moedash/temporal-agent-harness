...
Nexus at all: this demonstrates that starting the OpenAI Agents SDK plugin with `OpenAIAgentsPlugin(use_nexus_mcp_transport=True)` automatically uses Nexus to broker MCP tool calls where possible.
...
| `worker.py` | Worker on `default`. `OpenAIAgentsPlugin(use_nexus_mcp_transport=True)` is the only Nexus-related line. |
...
temporal nexus operation execute \
    --endpoint mcp-registry-endpoint --service RegistryService --operation RegisterExternal \
